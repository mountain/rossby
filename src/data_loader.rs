...
use crate::state::{Metadata, Variable, Dimension, AttributeValue, AppState};
...
fn validate_netcdf_data(metadata: &Metadata, data: &HashMap<String, Array<f32, IxDyn>>) -> Result<()> {
            ...
            if shape.len() != var.dimensions.len() {
                return Err(RossbyError::DataNotFound {
                    message: format!(
                        ...
                        var_name, var.dimensions.len(), shape.len()
                    ),
                });
            }
