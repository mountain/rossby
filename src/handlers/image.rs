...
fn parse_bbox(bbox_str: &str) -> Result<(f32, f32, f32, f32)> {
    ...
    let min_lon = parts[0].parse::<f32>().map_err(|_| RossbyError::InvalidParameter {
        param: "bbox".to_string(),
        message: "Could not parse min_lon as a float".to_string(),
    })?;
    
    let min_lat = parts[1].parse::<f32>().map_err(|_| RossbyError::InvalidParameter {
        param: "bbox".to_string(),
        message: "Could not parse min_lat as a float".to_string(),
    })?;
    
    let max_lon = parts[2].parse::<f32>().map_err(|_| RossbyError::InvalidParameter {
        param: "bbox".to_string(),
        message: "Could not parse max_lon as a float".to_string(),
    })?;
    
    let max_lat = parts[3].parse::<f32>().map_err(|_| RossbyError::InvalidParameter {
        param: "bbox".to_string(),
...
fn generate_image_response(state: Arc<AppState>, params: ImageQuery) -> Result<Response> {
    ...
    // Get output format
    let format = params.format.as_deref().unwrap_or(DEFAULT_FORMAT).to_lowercase();
