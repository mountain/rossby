...
fn process_point_query(state: Arc<AppState>, params: PointQuery) -> Result<PointResponse, RossbyError> {
    // Get coordinates
    let lon = params.lon;
    let lat = params.lat;
    ...
    let variables: Vec<String> = params.vars
        .split(',')
    ...
    for var_name in variables {
        ...
        let data_slice = data.as_slice().ok_or_else(|| RossbyError::DataNotFound {
            message: format!("Cannot access data for variable {} as contiguous slice", var_name),
        })?;
        ...
        // Add to results
        values.insert(var_name, serde_json::Value::Number(serde_json::Number::from_f64(value as f64).unwrap()));
    }
